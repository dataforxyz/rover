...
describe('detectOrphanedTasks', () => {
  ...
  it('marks stalled resume startup as FAILED when no replacement container id was recorded', async () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(
      expect.stringContaining('startup did not complete')
    );
  });

  it('skips missing-container resume startup while restart window is still in flight', async () => {
    const task = mockTask(32, 'IN_PROGRESS', undefined, {
