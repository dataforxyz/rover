...
const ERROR_PATTERNS: ErrorPattern[] = [
  ...
  {
    pattern:
      /authentication[_\s]error|invalid[_\s]bearer[_\s]token|\b401\b|unauthorized/i,
  ...
  // Permission errors
  {
    pattern: /permission[_\s]denied|access[_\s]denied|\bforbidden\b|\b403\b/i,
...
function extractJsonError(output: string): any {
  ...
  // Scan for JSON objects by tracking brace depth while skipping braces
  // inside quoted strings. This handles objects like {"msg": "use {braces}"}
  // correctly. The JSON.parse fallback ensures no false positives.
