...
import { computeLoopSignature } from '../../lib/step-executor.js';
...
const COMPLETED_DEPENDENCY_LOOP_STEP = {
  id: 'test_loop',
  type: 'loop',
  name: 'Test Loop',
  until: 'steps.run_tests.outputs.exit_code == 0',
...
describe('run command: loop checkpoint resume integration', () => {
  ...
  it('passes the recursive executable step count into agent runners for loop workflows', async () => {
    ...
    expect(constructorCalls[0][1]).toBe('step1');
    expect(constructorCalls[0][7]).toBe(4);
    expect(constructorCalls[0][8]).toBe(0);
    expect(constructorCalls[1][1]).toBe('fix_agent');
    expect(constructorCalls[1][7]).toBe(4);
    expect(constructorCalls[1][8]).toBe(2);
    expect(constructorCalls[2][1]).toBe('step3');
    expect(constructorCalls[2][7]).toBe(4);
    expect(constructorCalls[2][8]).toBe(3);
  });
      ...
      JSON.stringify(
        {
          ...
          loopSignatures: {
            test_loop: computeLoopSignature(TEST_LOOP_STEP as any),
          },
          loopProgress: {
            test_loop: {
              iteration: 1,
      ...
      JSON.stringify(
        {
          completedSteps: [{ id: 'step1', outputs: { result1: 'hello' } }],
          loopSignatures: {
            outer_loop: computeLoopSignature(OUTER_LOOP_STEP as any),
            inner_loop: computeLoopSignature(INNER_LOOP_STEP as any),
          },
          loopProgress: {
            outer_loop: {
              iteration: 1,
      ...
      JSON.stringify(
        {
          ...
          loopSignatures: {
            test_loop: computeLoopSignature(
              COMPLETED_DEPENDENCY_LOOP_STEP as any
            ),
          },
        },
        null,
  ...
  it('reruns a completed loop when the checkpoint signature is stale', async () => {
    writeFileSync(
      workflowPath,
      `
version: "1.0"
name: changed-loop
description: Resume should not skip edited loops
defaults:
  tool: codex
steps:
  - id: step1
  ...
  - id: test_loop
    type: loop
    name: Test Loop
    until: steps.run_lint.outputs.exit_code == 0
    maxIterations: 3
    steps:
      - id: run_tests
        type: command
  ...
  - id: step3
    type: agent
    name: Step 3
    if: steps.run_lint.outputs.exit_code == 0
    prompt: "Finalize after edited loop"
    outputs:
      - name: result3
        description: Final result
        type: string
`,
    ...
    const launchedCommands = vi
      .mocked(launch)
      .mock.calls.map(([, args]) => (Array.isArray(args) ? args.at(-1) : args));
    expect(launchedCommands[0]).toContain('npm test');
    expect(launchedCommands[1]).toContain('npm run lint');
    expect(runnerInstances.map(instance => instance.stepId)).toEqual(['step3']);
    expect(existsSync(checkpointPath)).toBe(false);
  });
});
