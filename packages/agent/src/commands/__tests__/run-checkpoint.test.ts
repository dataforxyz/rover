...
describe('checkpoint save/load', () => {
  ...
  it('should save and load loop signatures', () => {
    const data: CheckpointData = {
      completedSteps: [{ id: 'step1', outputs: { result: 'done' } }],
      loopSignatures: {
        review_loop: 'abc123',
    ...
    const loaded = loadCheckpoint(join(tempDir, 'checkpoint.json'));
    expect(loaded).not.toBeNull();
    expect(loaded!.loopSignatures).toEqual(data.loopSignatures);
  });

  it('should save and load external repository state', () => {
    const data: CheckpointData = {
      completedSteps: [{ id: 'step1', outputs: { result: 'done' } }],
    ...
    const loaded = loadCheckpoint(join(tempDir, 'checkpoint.json'));
    expect(loaded).not.toBeNull();
    expect(loaded!.externalRepositories).toEqual(data.externalRepositories);
  });
...
describe('createCheckpointStore', () => {
  ...
  it('updates in-memory loop progress without persisting to disk', () => {
    const store = createCheckpointStore('/nonexistent/dir/fails');
  ...
  it('updates in-memory completed steps without persisting to disk', () => {
    const store = createCheckpointStore('/nonexistent/dir/fails');
  ...
  it('persist() returns false when output directory is invalid', () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const store = createCheckpointStore('/nonexistent/dir/fails');

    store.addCompletedStep('s1', { result: 'done' });
  ...
  it('addCompletedStep appends a new step and persists on explicit persist()', () => {
    const store = createCheckpointStore(tempDir);
  ...
  it('addCompletedStep updates in-memory state without persisting', () => {
    const store = createCheckpointStore('/nonexistent/dir/fails');
