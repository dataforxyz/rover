...
describe('run command: pause/resume integration', () => {
  ...
  async function runAndCapture(
    opts: { agentTool?: string; checkpoint?: string; contextDir?: string } = {}
  ): Promise<number> {
  ...
  describe('Standard mode (Runner)', () => {
    ...
    it('copies agent logs when the workflow default tool is used', async () => {
      writeWorkflow();

      const agentLogSource = join(tempDir, 'agent-log-source');
      mkdirSync(agentLogSource, { recursive: true });
      writeFileSync(join(agentLogSource, 'agent.log'), 'workflow-default-log');
      vi.mocked(createAgent).mockReturnValue({
        getLogSources: () => [agentLogSource],
      ...
      expect(existsSync(join(outputDir, AGENT_LOGS_DIR, 'agent.log'))).toBe(
        true
      );
    });
  ...
  describe('Full pause → resume → complete lifecycle', () => {
    ...
    it('reinjects context when resuming from checkpoint', async () => {
      writeWorkflow();

      const contextDir = join(tempDir, 'context');
      mkdirSync(contextDir, { recursive: true });
      writeFileSync(join(contextDir, 'index.md'), '# Context index', 'utf8');
      ...
      const exitCode1 = await runAndCapture({ contextDir });
      ...
      (Runner as unknown as Mock).mockImplementation(() => {
        ...
        const instance = {
          run: vi.fn().mockResolvedValue(phase2Results[idx]),
        };
        runnerInstances.push(instance);
        return instance;
      });
      ...
      const exitCode2 = await runAndCapture({
      ...
      const resumedWorkflow = (Runner as unknown as Mock).mock.calls[0][0];
      expect(resumedWorkflow.getStep('step2').prompt).toContain(
        '**Context Sources:**'
      );
      expect(resumedWorkflow.getStep('step2').prompt).toContain(
        'Read the index file'
