...
describe('launchSync', () => {
  ...
  it('normalizes EPERM on the return path (non-thrown) when exitCode is 0', async () => {
    ...
    expect(result.code).toBeUndefined();
    // Original error messages are preserved (callers check failed/exitCode, not messages)
    expect(result.message).toBe('spawnSync echo EPERM');
    expect(result.shortMessage).toBe('Command failed with EPERM: echo world');
  });
