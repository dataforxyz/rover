...
import { SINGLE_CONDITION_REGEX } from 'rover-schemas';

/**
 * Capturing variant of SINGLE_CONDITION_REGEX.
 * Must stay in sync — both are derived from the same grammar:
 *   steps.<id>.outputs.<name> <op> <value>
 */
...
function splitOnLogicalOr(condition: string): string[] {
  const parts: string[] = [];
  // Match `||` followed by `steps.` with optional surrounding whitespace.
...
export function evaluateCondition(
  ...
  // Detect unsupported && (AND) operator between clauses.
  // Warn and return false instead of throwing to avoid breaking existing
  // persisted workflows that used && before schema validation rejected it.
...
function evaluateSingleCondition(
  ...
  // Validate and extract capture groups in a single pass.
  // Uses CAPTURING_CONDITION_REGEX which must stay aligned with
  // SINGLE_CONDITION_REGEX from the schema package.
