...
describe('TaskDescriptionManager', () => {
  ...
  describe('new status types', () => {
    ...
    it('clears stale error metadata when a resumed task completes', () => {
      ...
      task.markFailed('sandbox startup failed');
      expect(task.error).toBe('sandbox startup failed');
  ...
  describe('pause/resume lifecycle', () => {
    ...
    it('should support restart() from PAUSED status', () => {
      ...
      expect(task.autoRetryCount).toBe(0);
      expect(task.lastRestartAt).toBeDefined();
    });

    it('should reset persisted auto-retry count on resume transitions', () => {
      const taskPath = getTaskPath(28);
      const task = TaskDescriptionManager.create(taskPath, {
        id: 28,
        title: 'Reset Auto Retry Count',
        description: 'Test description',
        inputs: new Map(),
        workflowName: 'swe',
      });

      task.setAutoRetryCount(3);
      task.markPaused('Rate limit');
      task.markResuming();

      expect(task.autoRetryCount).toBe(0);

      const reloaded = TaskDescriptionManager.load(taskPath, 28);
      expect(reloaded.autoRetryCount).toBe(0);
    });
